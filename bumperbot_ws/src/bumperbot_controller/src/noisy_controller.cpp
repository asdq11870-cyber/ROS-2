...
NoisyController::NoisyController(const std::string &name) : Node(name),left_wheel_previous_position_(0.0),right_wheel_previous_position_(0.0),x_(0.0),y_(0.0),theta_(0.0){
    ...
    noisy_gen_ = std::default_random_engine(rd());

}

void NoisyController::jointCallback(const sensor_msgs::msg::JointState &msg){

    double left_wheel_encoder = msg.position.at(1) + left_encoder_noise_(noisy_gen_);
    double right_wheel_encoder = msg.position.at(0) + right_encoder_noise_(noisy_gen_);
    double dp_left = left_wheel_encoder - left_wheel_previous_position_;
    double dp_right = right_wheel_encoder - right_wheel_previous_position_;
    ...
    double angular_vel_left = dp_left / dt.seconds();
    double angular_vel_right = dp_right / dt.seconds();

    double bot_linear = (wheel_radius_/2)*(angular_vel_left+angular_vel_right);
    double bot_angular = (wheel_radius_/wheel_separation_)*(angular_vel_right-angular_vel_left);

    double d_s = (wheel_radius_/2)*(dp_left+dp_right);
    ...
    odom_msg.twist.twist.linear.x = bot_linear;
    odom_msg.twist.twist.angular.z = bot_angular;
    ...
    //RCLCPP_INFO_STREAM(get_logger(),"Linear velocity: " << bot_linear << " Angular velocity: " << bot_angular);
