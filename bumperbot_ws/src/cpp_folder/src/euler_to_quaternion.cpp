#include "cpp_folder/euler_to_quaternion.hpp"

using namespace std::chrono_literals;
using namespace std::placeholders;

SimpleTFKinematics::SimpleTFKinematics(const std::string &name) : Node(name),x_increment_(0.05),last_x_(0.0),rotation_counter_(0){
    ...
    get_transform_srv_ = create_service<bumperbot_msgs::srv::GetTransform>("get_transform",std::bind(&SimpleTFKinematics::responseTransformCallback,this,_1,_2));
...
bool SimpleTFKinematics::responseTransformCallback(const std::shared_ptr<bumperbot_msgs::srv::GetTransform::Request> request, const std::shared_ptr<bumperbot_msgs::srv::GetTransform::Response> response){
    ...
    try{
        ...
        RCLCPP_ERROR_STREAM(get_logger(),"An error occured during the transformation: " << e.what());
