...
public:
    SimpleServiceClient(int a, int b) : Node("simple_service_client"){
        ...
        auto result = client_->async_send_request(request, std::bind(&SimpleServiceClient::clientCallback,this,_1));
...
private:
    ...
    void clientCallback(rclcpp::Client<bumperbot_msgs::srv::AddTwoInts>::SharedFuture future){
        if(future.valid()){
