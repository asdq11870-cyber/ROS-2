#include "bumperbot_localisation/kalman_filter.hpp"

using std::placeholders::_1;

KalmanFilter::KalmanFilter(const std::string &name) : Node(name),mean_(0.0),variance_(0.0),imu_angular_z_(0.0),is_first_odom_(true),last_angular_z_(0.0),motion_(0.0),motion_variance_(4.0),measurement_variance_(0.5){
...
void KalmanFilter::measurementUpdate(){
    ...
    variance_ = 1/((1/measurement_variance_)+(1/variance_));
}

int main(int argc, char* argv[]){
    rclcpp::init(argc,argv);
    auto node = std::make_shared<KalmanFilter>("kalman_filter_node");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}